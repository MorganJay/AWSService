    ...
    public class UploadService : IUploadService
    {
        ...
        public async Task<AWSUploadResult<string>> UploadImageToS3BucketAsync(UploadRequestDto requestDto)
        {
            try
            {
                ...
                var trustedStorageName = "images/" + randomFileName + ext;
        ...
        public AWSUploadResult<string> GenerateAwsFileUrl(string bucketName, string key, bool useRegion = true)
        {
            ...
            if (useRegion)
            {
                publicUrl = $"https://{bucketName}.s3.{_config.AwsRegion}.{_config.AwsS3BaseUrl}/{key}";
            }
            else
            {
                publicUrl = $"https://{bucketName}.s3.{_config.AwsS3BaseUrl}/{key}";
            }
        ...
        public async Task<(bool, List<S3Object>)> ListBucketContent(string bucketName, string folder = "")
        {
            var request = new ListObjectsRequest
            {
                BucketName = bucketName,
                MaxKeys = 10,
                Prefix = folder is "" ? null : folder
            };
            ...
            //    list.Add(item.Key);
            //}
            return (response.HttpStatusCode == HttpStatusCode.OK, response.S3Objects);
        }

        public async Task<(bool, GetObjectResponse)> GetS3Object(string bucketName, string key)
        {
            ...
            var response = await _amazonS3Client.GetObjectAsync(bucketName, key);
